...
import sys
from tkinter import Tk, StringVar, Label, Checkbutton, Label
...
def affichage():
    ...
    labl1 = Label(root, afficheTableau())
...
try :
    ...
    while not success:
        ...
        bouton_retour = Button(root,text="Retour",input( "00"))
        print('00 - Retour /')
        afficheTableau(r,max(1,deb),min(fin, len(r)))
        bouton_precedent=Button(root,text="Precedent,",input("98"))
        print('98 - Voir les liens précédents /')
        bouton_suivant=Button(root,text="Suite des liens,",input("99"))
